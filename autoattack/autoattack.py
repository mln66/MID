...
class AutoAttack():
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        with torch.no_grad():
            ...
            robust_flags = torch.zeros(x_orig.shape[0], dtype=torch.uint8, device=x_orig.device)
            ...
            for attack in self.attacks_to_run:
                ...
                robust_lin_idcs = torch.nonzero(robust_flags)
                if num_robust > 1:
                ...
                for batch_idx in range(n_batches):
                    ...
                    robust_flags[non_robust_lin_idcs] = 0
