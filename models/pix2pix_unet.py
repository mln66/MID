...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        feature = out['bottle'].view(out['bottle'].size(0), -1)
        predict = self.cls(self.fc(feature))
        out['u1'] = self.up1(out['bottle'])
        out['u2'] = self.up2(out['u1'])
        return self.outc(out['u2']), predict
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            target_tensor = self.real_label  # 全是1
        else:
            target_tensor = self.fake_label  # 全是0
...
class ResnetGenerator_classifier(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(ResnetGenerator_classifier, self).__init__()
        self.fc = nn.Linear(256*64, 1024)
        self.cls = nn.Linear(1024, 10)
